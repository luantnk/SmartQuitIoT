...
import { Upload, Save, X, Trophy } from "lucide-react";
...
const CreateAchievement = () => {
  ...
  const onSubmit = async (data) => {
    if (!iconFile) {
      toast.error("Please upload an achievement icon");
      return;
    }
    ...
    try {
      ...
      setError(error?.response?.data?.message || "Failed to create achievement");
  ...
  return (
    <div className="p-6 space-y-6">
      <AppBreadcrumb paths={["admin", "manage-achievements", "create-achievement"]} />

      <Card>
        <CardHeader>
          <CardTitle>Create New Achievement</CardTitle>
          <CardDescription>
            Add a new achievement to the system. Fill in all the required information below.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                  ...
                  <label className="flex items-center justify-center w-full px-4 py-6 border-2 border-dashed border-border rounded-lg cursor-pointer hover:bg-muted/50 transition-colors">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <span className="text-sm text-muted-foreground">
                        Click to upload achievement icon
                      </span>
