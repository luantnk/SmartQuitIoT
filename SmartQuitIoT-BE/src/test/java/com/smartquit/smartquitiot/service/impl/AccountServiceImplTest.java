...
class AccountServiceImplTest {
  ...
  void should_throw_exception_when_email_already_exists() {
    // ===== GIVEN =====
    when(accountRepository.findByEmail(memberRequest.getEmail())).thenReturn(Optional.of(account));
  ...
  void should_throw_exception_when_reset_token_not_found() {
    ...
    when(accountRepository.findByResetToken(request.getResetToken())).thenReturn(Optional.empty());
