...
class AgoraServiceImplTest {
  ...
  @InjectMocks private AgoraServiceImpl agoraService;
  ...
  void should_throw_exception_when_agora_sdk_fails() {
    try (MockedConstruction<RtcTokenBuilder2> mocked =
        mockConstruction(
            RtcTokenBuilder2.class,
            (mock, context) ->
                when(mock.buildTokenWithUid(
                        anyString(), anyString(), anyString(), anyInt(), any(), anyInt(), anyInt()))
                    .thenThrow(new RuntimeException("SDK error")))) {

      assertThatThrownBy(() -> agoraService.generateRtcToken("channel", 100, 3600))
