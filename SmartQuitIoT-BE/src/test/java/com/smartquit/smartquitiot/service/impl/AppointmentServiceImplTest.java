...
class AppointmentServiceImplTest {
  ...
  @Mock private CoachWorkScheduleRepository coachWorkScheduleRepository;
  ...
  void should_throw_exception_when_member_not_found() {
    ...
    when(memberRepository.findByAccountId(accountId)).thenReturn(Optional.empty());
  ...
  void should_cancel_appointment_successfully_when_member_cancels() {
    ...
    ZonedDateTime now =
        appointmentZoned.minusMinutes(10); // Cancel 10 phút trước (hợp lệ, cần > 5 phút)
  ...
  void should_throw_exception_when_member_not_authorized_to_cancel() {
    ...
    when(memberRepository.findByAccountId(accountId)).thenReturn(Optional.of(otherMember));
  ...
  //
  //        // ===== WHEN & THEN =====
  //        assertThatThrownBy(() -> appointmentService.completeAppointmentByCoach(appointmentId,
  // coachAccountId))
  //                .isInstanceOf(IllegalStateException.class)
  ...
  void should_get_appointment_detail_for_member() {
    ...
    AppointmentResponse response = new AppointmentResponse();
    when(appointmentMapper.toResponseWithRuntime(any(), anyString())).thenReturn(response);

    // ===== WHEN =====
    AppointmentResponse result =
        appointmentService.getAppointmentDetailForPrincipal(appointmentId, accountId);
