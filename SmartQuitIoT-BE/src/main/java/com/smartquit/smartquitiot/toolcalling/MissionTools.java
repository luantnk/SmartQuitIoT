...
public class MissionTools {
  ...
  public List<PhaseDetailMissionToolDTO> getCandidateMissions(
      @ToolParam(
              description =
                  "Enum value of the target phase (PREPARATION, ONSET, PEAK_CRAVING, SUBSIDING,"
                      + " MAINTENANCE).")
    ...
    QuitPlan plan =
        quitPlanRepository
            .findById(planId)
            .orElseThrow(() -> new RuntimeException("Mission Plan ID " + planId + " not found."));
    ...
    List<Mission> candidates =
        missionService.filterMissionsForPhase(plan, account, missionPhase, MissionStatus.ACTIVE);
    log.info("Filtered {} candidate missions for phase: {}", candidates.size(), missionPhase);
    log.info("Candidate codes: {}", candidates.stream().map(Mission::getCode).toList());
