...
/**
 * WS controller for /app/conversations/messages - Primary flow: jwtChannelInterceptor should set
 * Principal at CONNECT - Fallback: try read Authorization from the frame's native headers and
 * decode with JwtDecoder
 *
 * <p>NOTE: fallback only helps debugging/dev. Production: rely on interceptor.
 */
@Controller
...
public class ChatWebSocketController {
  ...
  public void onMessage(
      @Payload WsMessageRequest payload,
    ...
    try {
      ...
      if (principal == null || SecurityContextHolder.getContext().getAuthentication() == null) {
        ...
        if (rawAuth == null) {
          log.warn(
              "STOMP message received without principal - no Authorization header found in frame"
                  + " (session={})",
              headerAccessor.getSessionId());
          // try to notify FE: if FE provided username/accountId header, use that
          sendErrorToUserByHeader(
              headerAccessor,
              "Not authenticated. Please include Authorization Bearer token in STOMP CONNECT"
                  + " headers.");
          return;
        }
        ...
        if (token.isEmpty()) {
          log.warn(
              "STOMP message received without principal - empty token after strip (session={})",
              headerAccessor.getSessionId());
        ...
        try {
          ...
          setTempAuth = true;
          log.debug(
              "[FallbackAuth] set temporary Authentication principal={} authorities={} session={}",
              principalName,
              authorities,
              headerAccessor.getSessionId());
        } catch (Exception ex) {
      ...
      log.info(
          "[WS] recv from principal={} conv={} clientMessageId={}",
          principalName,
      ...
      log.debug(
          "WS: message persisted id={} conv={}",
          saved != null ? saved.getId() : null,
          saved != null ? saved.getConversationId() : null);
      ...
      if (saved != null) {
        try {
          ...
          log.info(
              "[WS DEBUG] echoing to topic {} clientMessageId={}", topic, req.getClientMessageId());
      ...
      sendErrorToUser(
          SecurityContextHolder.getContext().getAuthentication(), "Forbidden: " + ex.getMessage());
    } catch (Exception ex) {
      ...
      try {
        if (setTempAuth) {
          ...
          log.debug(
              "[FallbackAuth] restored previous SecurityContext (session={})",
              headerAccessor.getSessionId());
        }
      } catch (Exception e) {
  ...
  private String extractAuthFromAccessor(SimpMessageHeaderAccessor accessor) {
    try {
      ...
      if (nh instanceof Map<?, ?>) {
        ...
        for (String key :
            List.of("Authorization", "authorization", "Auth", "auth", "token", "accessToken")) {
  ...
  private void sendErrorToUserByHeader(SimpMessageHeaderAccessor headerAccessor, String message) {
    try {
      ...
      try {
        possible = headerAccessor.getNativeHeader("username");
      } catch (Throwable ignored) {
      }
      if (possible == null || possible.isEmpty()) {
        try {
          possible = headerAccessor.getNativeHeader("accountId");
        } catch (Throwable ignored) {
        }
      }
      if (possible != null && !possible.isEmpty()) {
