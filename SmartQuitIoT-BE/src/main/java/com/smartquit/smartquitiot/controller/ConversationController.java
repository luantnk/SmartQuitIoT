...
public class ConversationController {
  ...
  @Operation(
      summary = "Gửi tin nhắn (tự tạo conversation DIRECT nếu chưa có)",
      description =
          "Nếu conversationId được truyền sẽ gửi vào đó; nếu không thì cần targetUserId để tạo/find"
              + " direct conversation.")
  ...
  public ResponseEntity<GlobalResponse<MessageDTO>> sendMessage(
    ...
    try {
      MessageDTO dto =
          messageService.sendMessage(req); // service resolves current user from SecurityContext
      ...
      return ResponseEntity.status(500)
          .body(GlobalResponse.error("Server error: " + e.getMessage(), 500));
    }
  }
  ...
  public ResponseEntity<GlobalResponse<List<ConversationSummaryDTO>>> listConversations(
    ...
    try {
      List<ConversationSummaryDTO> list =
          conversationService.listConversationsForCurrentUser(page, size);
