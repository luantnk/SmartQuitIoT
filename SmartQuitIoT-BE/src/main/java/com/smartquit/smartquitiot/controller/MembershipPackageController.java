...
public class MembershipPackageController {
  ...
  public ResponseEntity<GlobalResponse<List<MembershipPackageDTO>>> getMembershipPackages() {
    return ResponseEntity.ok(
        GlobalResponse.ok(
            "Get Membership Packages", membershipPackageService.getMembershipPackages()));
  }

  @GetMapping("/plans/{membershipPackageId}")
  @Operation(
      summary = "Get all membership package plans by Id",
      description = "Chỉ có gói Standard và Premium có plan 1 tháng với 12 tháng")
  public ResponseEntity<?> getMembershipPackagePlans(@PathVariable int membershipPackageId) {
    return ResponseEntity.ok(
        membershipPackageService.getMembershipPackagesPlanByMembershipPackageId(
            membershipPackageId));
  }

  @PostMapping("/create-payment-link")
  @Operation(
      summary = "Create membership payment link",
      description =
          "Mobile gửi membershipPackageId, duration về, nếu id của gói free trial sẽ không tạo"
              + " payment link mà sẽ tạo MembershipSubscription với gói Free Trial luôn.")
  @SecurityRequirement(name = "Bearer Authentication")
  public ResponseEntity<?> createMembershipPaymentLink(
      @RequestBody MembershipPaymentRequest request) {
    return new ResponseEntity<>(
        membershipPackageService.createMembershipPackagePayment(
            request.getMembershipPackageId(), request.getDuration()),
        HttpStatus.CREATED);
  }
  ...
  @Operation(
      ...
      description =
          "Mobile gửi body gồm các trường như bên dưới để BE xác thực thanh toán , xử lí gói"
              + " membership")
  public ResponseEntity<GlobalResponse<MembershipSubscriptionDTO>> processMembershipPayment(
      @RequestBody PaymentProcessRequest request) {
    return ResponseEntity.ok(
        GlobalResponse.created(
            "Membership payment success",
            membershipPackageService.processMembershipPackagePayment(request)));
  }
  ...
  public ResponseEntity<?> getMembershipPackageDetails(@PathVariable int membershipPackageId) {
    return ResponseEntity.ok(
        membershipPackageService.getMembershipPackageDetails(membershipPackageId));
  }
  ...
  public ResponseEntity<GlobalResponse<MembershipPackageDTO>> updateMembershipPackage(
      @RequestBody UpdateMembershipPackageRequest request) {
    return ResponseEntity.ok(
        GlobalResponse.ok(
            "Update membership package success",
            membershipPackageService.updateMembershipPackage(request)));
  }
  ...
  public ResponseEntity<GlobalResponse<List<MembershipPackageDTO>>> manageMembershipPackages() {
    return ResponseEntity.ok(
        GlobalResponse.ok(
            "Get Membership Packages", membershipPackageService.manageMembershipPackages()));
  }
}
