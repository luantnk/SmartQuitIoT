...
public class MembershipSubscriptionController {
  ...
  @GetMapping("/current")
  @Operation(
      summary = "Get current membership package",
      description = "Api này trả về gói membership đang hữu dụng hiện tại")
  @SecurityRequirement(name = "Bearer Authentication")
  public ResponseEntity<GlobalResponse<MembershipSubscriptionDTO>>
      getCurrentMembershipSubscription() {
    return ResponseEntity.ok(
        GlobalResponse.ok(
            "Get current membership package success",
            membershipSubscriptionService.getMyMembershipSubscription()));
  }
  ...
  public ResponseEntity<Page<MembershipSubscriptionDTO>> getAllMembershipSubscription(
    ...
    return ResponseEntity.ok(
        membershipSubscriptionService.getAllMembershipSubscriptions(
            page, size, sortBy, sortDir, orderCode, status));
  }

  @GetMapping("/member/{memberId}")
  @Operation(
      summary = "Get membership packages by memberId",
      description = "Api này trả về gói membership theo memberId")
  public ResponseEntity<List<MembershipSubscriptionDTO>> getAllMembershipSubscription(
      @PathVariable int memberId) {
    return ResponseEntity.ok(
        membershipSubscriptionService.getMembershipSubscriptionsByUserId(memberId));
  }
}
