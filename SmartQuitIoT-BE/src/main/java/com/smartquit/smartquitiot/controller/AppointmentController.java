...
public class AppointmentController {
  ...
  public ResponseEntity<GlobalResponse> bookAppointment(
      @Valid @RequestBody AppointmentRequest request, @AuthenticationPrincipal Jwt jwt) {

    int accountId = getAccountId(jwt);
    log.info(
        "REST request to book appointment for accountId: {} on date: {}",
        accountId,
        request.getDate());
  ...
  public ResponseEntity<GlobalResponse> cancelByMember(
      @PathVariable int appointmentId, @AuthenticationPrincipal Jwt jwt) {
  ...
  public ResponseEntity<GlobalResponse> listForCurrentUser(
    ...
    log.debug(
        "REST request to list appointments for accountId: {} (Role: {})",
        accountId,
        isCoach ? "COACH" : "MEMBER");

    Object res =
        isCoach
            ? appointmentService.getAppointmentsByCoachAccountId(
                accountId, status, date, page, size)
            : appointmentService.getAppointmentsByMemberAccountId(
                accountId, status, date, page, size);
  ...
  public ResponseEntity<GlobalResponse> getJoinToken(
    ...
    int accountId = getAccountId(jwt);
    log.info(
        "Generating Agora Join Token for appointment: {} by account: {}", appointmentId, accountId);

    JoinTokenResponse dto =
        appointmentService.generateJoinTokenForAppointment(appointmentId, accountId);
  ...
  public ResponseEntity<GlobalResponse> completeByCoach(
    ...
    int accountId = getAccountId(jwt);
    log.info(
        "COACH ACCESS: Marking appointment {} as COMPLETED by coach: {}", appointmentId, accountId);
  ...
  public ResponseEntity<GlobalResponse> reassignAppointment(
      @PathVariable int appointmentId, @RequestBody ReassignAppointmentRequest request) {

    log.info(
        "ADMIN ACCESS: Reassigning appointment: {} to new coach: {}",
        appointmentId,
  ...
  /**
   * Helper to extract accountId and log errors if missing. In a real project, this logic could be
   * moved to a custom ArgumentResolver.
   */
