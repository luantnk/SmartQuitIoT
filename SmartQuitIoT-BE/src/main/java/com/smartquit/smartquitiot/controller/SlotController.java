...
public class SlotController {
  ...
  public ResponseEntity<GlobalResponse<Page<SlotDTO>>> getAllSlots(
      @RequestParam(name = "page", defaultValue = "0") int page,
      @RequestParam(name = "size", defaultValue = "10") int size) {
    return ResponseEntity.ok(
        GlobalResponse.ok("Get all slot success", slotService.listAllSlots(page, size)));
  }
  ...
  @Operation(
      ...
      description =
          "Reseed slots with new start time, end time, slot duration, and gap. "
  ...
  public ResponseEntity<GlobalResponse<SlotReseedResponse>> reseedSlots(
    ...
    SlotReseedResponse response =
        slotService.reseedSlots(start, end, request.getSlotMinutes(), request.getGapMinutes());

    String message =
        String.format(
            "Slots reseeded successfully. Created: %d slots, Deleted: %d orphan slots",
            response.getCreatedCount(), response.getDeletedCount());

    return ResponseEntity.ok(GlobalResponse.ok(message, response));
  }

  /** Normalize time format from "H:mm" to "HH:mm" Example: "5:30" -> "05:30", "09:30" -> "09:30" */
