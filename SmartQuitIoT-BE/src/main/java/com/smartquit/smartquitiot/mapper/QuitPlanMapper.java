...
public class QuitPlanMapper {
  ...
  public QuitPlanResponse toResponse(QuitPlan plan) {
    ...
    if (plan.getPhases() != null) {
      List<PhaseDTO> phaseDTOs = plan.getPhases().stream().map(this::toPhaseDTO).toList();
  ...
  private PhaseDTO toPhaseDTO(Phase phase) {
    ...
    if (phase.getDetails() != null) {
      List<PhaseDetailResponseDTO> details =
          phase.getDetails().stream().map(this::toPhaseDetailDTO).toList();
  ...
  private PhaseDetailResponseDTO toPhaseDetailDTO(PhaseDetail detail) {
    ...
    if (detail.getPhaseDetailMissions() != null) {
      List<PhaseDetailMissionResponseDTO> missions =
          detail.getPhaseDetailMissions().stream().map(this::toMissionDTO).toList();
