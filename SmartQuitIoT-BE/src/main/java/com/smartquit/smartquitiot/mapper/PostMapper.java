...
public class PostMapper {
  ...
  public static PostDetailDTO toDetailDTO(Post post, int commentCount) {
    ...
    if (post.getMedia() != null) {
      List<PostDetailDTO.PostMediaDTO> mediaDTO =
          post.getMedia().stream()
              .map(
                  m ->
                      new PostDetailDTO.PostMediaDTO(
                          m.getId(), m.getMediaUrl(), m.getMediaType().name()))
    ...
    if (post.getComments() != null) {
      List<PostDetailDTO.CommentDTO> commentDTOs =
          post.getComments().stream().map(PostMapper::toCommentDTO).collect(Collectors.toList());
  ...
  private static PostDetailDTO.CommentDTO toCommentDTO(Comment comment) {
    ...
    if (comment.getCommentMedia() != null) {
      List<PostDetailDTO.PostMediaDTO> mediaDTO =
          comment.getCommentMedia().stream()
              .map(
                  m ->
                      new PostDetailDTO.PostMediaDTO(
                          m.getId(), m.getMediaUrl(), m.getMediaType().name()))
    ...
    if (comment.getReplies() != null) {
      List<PostDetailDTO.CommentDTO> repliesDTO =
          comment.getReplies().stream().map(PostMapper::toCommentDTO).collect(Collectors.toList());
  ...
  public static PostDetailDTO toDTO(Post post) {
    ...
    if (post.getMedia() != null && !post.getMedia().isEmpty()) {
      List<PostDetailDTO.PostMediaDTO> mediaDTOs =
          post.getMedia().stream()
              .map(
                  media -> {
