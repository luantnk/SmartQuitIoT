...
public class AuthenticationServiceImpl implements AuthenticationService {
  ...
  public AuthenticationResponse loginWithGoogle(String idTokenString)
    ...
    account =
        existingUser.orElseGet(
            () -> createNewGoogleAccount(email, firstName, lastName, pictureUrl));
    if (!account.isActive()) {
      throw new RuntimeException("Account is not active");
    }
    if (account.isBanned()) {
  ...
  private Account createNewGoogleAccount(
  ...
  private String generateAccessToken(Account account) {
    ...
    JWTClaimsSet.Builder claimsBuilder =
        new JWTClaimsSet.Builder()
            ...
            .issueTime(new Date())
            .expirationTime(
                new Date(Instant.now().plus(accessTokenDuration, ChronoUnit.HOURS).toEpochMilli()))
            .jwtID(UUID.randomUUID().toString())
            .claim(
                "scope", account.getRole() != null ? account.getRole().name() : Role.MEMBER.name())
  ...
  private String generateRefreshToken(Account account) {
    ...
    JWTClaimsSet claimsSet =
        new JWTClaimsSet.Builder()
            ...
            .issueTime(new Date())
            .expirationTime(
                new Date(Instant.now().plus(refreshTokenDuration, ChronoUnit.DAYS).toEpochMilli()))
            .jwtID(UUID.randomUUID().toString())
            .build();
  ...
  private SignedJWT verifyAndParseToken(String token, String key)
      throws ParseException, JOSEException {
