...
public class ConversationServiceImpl implements ConversationService {
  ...
  public List<ConversationSummaryDTO> listConversationsForCurrentUser(int page, int size) {
    ...
    List<Conversation> convs =
        conversationRepository.findAllByParticipantAccountId(
    ...
    return convs.stream()
        .map(
            conv -> {
              ...
              //  lastReadAt
              var participantOpt =
                  participantRepository.findByConversationIdAndAccountId(
                      conv.getId(), current.getId());
              LocalDateTime lastReadAt =
                  participantOpt.map(Participant::getLastReadAt).orElse(null);

              long unread = messageRepository.countUnread(conv.getId(), lastReadAt);

              // participants summary
              List<ParticipantSummaryDTO> parts =
                      ...
                      .map(
                          p -> {
                            ...
                            if ((fullName == null || fullName.isEmpty())
                              ...
                              String fn =
                                  coach.getFirstName() == null ? "" : coach.getFirstName().trim();
                              String ln =
  ...
  public ConversationSummaryDTO markConversationRead(int conversationId) {
    ...
    Participant participant =
        participantRepository
            .findByConversationIdAndAccountId(conversationId, current.getId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException("You are not a participant of this conversation"));
  ...
  private LastMessageDTO mapToLastMessageDTO(Message m) {
    ...
    return new LastMessageDTO(
        m.getId(),
        m.getSender() != null ? m.getSender().getId() : 0,
        m.getMessageType(),
        m.getContent(),
        m.getSentAt());
  }
