...
public class PostServiceImpl implements PostService {
  ...
  private boolean isContentFlagged(
      String title, String description, String content, String thumbnail) {
    try {
      ...
      log.warn(
          "AI Service check failed for content. Defaulting to FLAGGED (PENDING_APPROVAL). Error:"
              + " {}",
          e.getMessage());
      return true;
    }
  ...
  private boolean isMediaFlagged(String url, String mediaType) {
    try {
      ...
      log.warn(
          "AI Service check failed for media URL: {}. Defaulting to FLAGGED. Error: {}",
          url,
          e.getMessage());
      return true;
    }
  ...
  public List<PostSummaryDTO> getAllPosts(String query) {
    if (!StringUtils.hasText(query)) {
      ...
      return searchResults.stream().map(this::mapDocumentToSummaryDTO).toList();
    }
  }
  ...
  public PostDetailDTO createPost(PostCreateRequest request) {
    ...
    if (isFlagged) {
      ...
      log.warn(
          "Post created by {} flagged as TOXIC/NSFW (or AI Error). Status set to PENDING_APPROVAL.",
          current.getUsername());
    } else {
    ...
    if (savedPost.getStatus() == PostStatus.PUBLISHED) {
      ...
      if (current.getRole().equals(Role.MEMBER)) {
        metricRepository
            .findByMemberId(current.getMember().getId())
            .ifPresent(
                metric -> {
  ...
  public PostDetailDTO updatePost(Integer postId, PostUpdateRequest request) {
    ...
    boolean isFlagged =
        isContentFlagged(
            post.getTitle(), post.getDescription(), post.getContent(), post.getThumbnail());
  ...
  public void syncAllPosts() {
    ...
    List<PostDocument> documents =
        allPosts.stream().map(this::mapToDocument).collect(Collectors.toList());
