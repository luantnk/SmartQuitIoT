...
public class MessageServiceImpl implements MessageService {
  ...
  public MessageDTO sendMessage(MessageCreateRequest req) {
    ...
    if (req.getConversationId() == null
        && req.getTargetMemberId() == null
        && req.getTargetUserId() == null) {
      throw new IllegalArgumentException(
          "Either conversationId or (targetMemberId or targetUserId) must be provided");
    }
    ...
    if (req.getConversationId() != null) {
      ...
      boolean isParticipant =
          participantRepository.findByConversationAndAccount(conv, sender).isPresent();
      if (!isParticipant) {
              ...
              conv.getParticipants() != null
                  && conv.getParticipants().stream()
                      .anyMatch(
                          p ->
                              p.getAccount() != null
                                  && Objects.equals(p.getAccount().getId(), sender.getId()));
      ...
      // resolve targetAccountId: priority -> targetMemberId (member -> account) -> targetUserId
      // (accountId)
      Integer targetAccountId;
      if (req.getTargetMemberId() != null) {
        Member targetMember =
            memberRepository
                .findById(req.getTargetMemberId())
                .orElseThrow(
                    () ->
                        new IllegalArgumentException(
                            "Target member not found: " + req.getTargetMemberId()));
      ...
      Account target =
          accountRepository
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException("Target account not found: " + targetAccountId));

      Optional<Conversation> convOpt =
          conversationRepository.findDirectConversationBetween(sender.getId(), targetAccountId);
    ...
    if (req.getAttachments() != null && !req.getAttachments().isEmpty()) {
      List<Attachment> attachmentEntities =
    ...
    log.info(
        "[MSG] prepared to broadcast after commit convId={} msgId={} clientMessageId={}",
        convId,
        savedFinal.getId(),
        clientMessageIdFinal);
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
          ...
          new TransactionSynchronization() {
            ...
            public void afterCommit() {
              try {
                ...
                log.info(
                    "[MSG] afterCommit broadcasting to /topic/conversations/{} payload={}",
                    convId,
                    payload);
                messagingTemplate.convertAndSend("/topic/conversations/" + convId, payload);
                log.info(
                    "[MSG] broadcast done convId={} clientMessageId={}",
                    convId,
                    clientMessageIdFinal);
              } catch (Exception ex) {
                log.error(
                    "[MSG] broadcast error convId={} clientMessageId={}",
                    convId,
                    clientMessageIdFinal,
                    ex);
              }
      ...
      try {
        ...
        log.info(
            "[MSG] no-tx broadcasting to /topic/conversations/{} payload={}",
            conv.getId(),
            payload);
  ...
  public List<MessageDTO> getMessages(int conversationId, Integer beforeId, int limit) {
    ...
    boolean isParticipant =
        participantRepository.findByConversationAndAccount(conv, current).isPresent();
    if (!isParticipant) {
      throw new SecurityException("Not participant of this conversation");
    }

    List<Message> messages =
        messageRepository.findMessagesByConversationBeforeId(
