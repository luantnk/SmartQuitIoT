...
public class ScheduleServiceImpl implements ScheduleService {
  ...
  public int assignCoachesToDates(ScheduleAssignRequest request) {
    ...
    Set<LocalDate> parsedDates =
        request.getDates().stream()
            .filter(Objects::nonNull)
            .collect(Collectors.toCollection(LinkedHashSet::new));

    Set<Integer> coachIdSet =
        request.getCoachIds().stream()
            .filter(Objects::nonNull)
            .collect(Collectors.toCollection(LinkedHashSet::new));
    ...
    for (LocalDate date : parsedDates) {
      for (Integer coachId : coachIdSet) {
        ...
        Set<Integer> existingSlotIds =
            existing.stream().map(c -> c.getSlot().getId()).collect(Collectors.toSet());
    ...
    if (!newSchedules.isEmpty()) {
      try {
        ...
        log.warn(
            "DataIntegrityViolationException when saving schedules (likely duplicates due to"
                + " concurrent inserts): {}",
            ex.getMessage());
  ...
  public List<ScheduleByDayResponse> getSchedulesByMonth(int year, int month) {
    ...
    //            log.info("Requested month {}-{} is in the past — returning empty result", year,
    // month);
    //            return List.of();
    //        }

    LocalDate start = requested.equals(current) ? today : requested.atDay(1);
    ...
    // Group by date
    Map<LocalDate, List<CoachWorkSchedule>> grouped =
        schedules.stream().collect(Collectors.groupingBy(CoachWorkSchedule::getDate));

    List<ScheduleByDayResponse> result = new ArrayList<>();

    grouped.forEach(
        (date, list) -> {
          List<CoachSummaryDTO> coachList =
              list.stream()
          ...
          if (!coachList.isEmpty()) {
            result.add(ScheduleByDayResponse.builder().date(date).coaches(coachList).build());
          }
        });
  ...
  public void updateScheduleByDate(LocalDate date, ScheduleUpdateRequest request) {
    ...
    for (Integer coachId : request.getRemoveCoachIds()) {
      ...
      boolean allAvailable =
          schedules.stream().allMatch(s -> s.getStatus() == CoachWorkScheduleStatus.AVAILABLE);

      if (!allAvailable) {
        throw new IllegalStateException(
            String.format(
                "Cannot remove coach %d from date %s because some slots are not available (may be"
                    + " booked or in progress).",
                coachId, date));
      }
