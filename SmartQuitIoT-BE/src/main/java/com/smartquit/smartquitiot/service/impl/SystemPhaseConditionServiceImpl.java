...
public class SystemPhaseConditionServiceImpl implements SystemPhaseConditionService {
  ...
  @Override
  @Transactional
  public SystemPhaseConditionDTO updateSystemPhaseCondition(
      Integer id, UpdateSystemPhaseConditionRequest req) {
    SystemPhaseCondition phaseCondition =
        systemPhaseConditionRepository
            .findById(id)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "System phase condition not found with id: " + id));
  ...
  private boolean evaluateConditionForTest(
      JsonNode node, TestData testData, List<RuleEvaluationDetail> ruleResults) {
  ...
  private String buildEvaluationDetails(
      JsonNode condition, TestData testData, boolean passed, List<RuleEvaluationDetail> results) {
  ...
  private void validateConditionStructure(JsonNode condition) {
    if (condition == null || !condition.has("logic") || !condition.has("rules")) {
      throw new IllegalArgumentException(
          "Invalid condition structure: must have 'logic' and 'rules' fields");
    }
      ...
      if (rule.has("rules")) {
        ...
        if (rule.has("formula")) {
          ...
          if (!formula.has("base") || !formula.has("operator") || !formula.has("percent")) {
            throw new IllegalArgumentException(
                "Formula must have 'base', 'operator', and 'percent' fields");
          }
        }
      }
    }
