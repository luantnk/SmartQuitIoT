...
public class AccountServiceImpl implements AccountService {
  ...
  public void forgotPassword(String email) {
    ...
    EmailMessageDTO emailMessage = new EmailMessageDTO(email, subject, "otp", props);
  ...
  public VerifyOtpResponse verifyOtp(VerifyOtpRequest request) {
    ...
    if (Duration.between(account.getOtpGeneratedTime(), LocalDateTime.now()).toMinutes()
        > OTP_VALID_DURATION_MINUTES) {
      throw new IllegalArgumentException("OTP has expired. Please request a new one.");
    }
    String token = UUID.randomUUID().toString();
    account.setResetToken(token);
    account.setResetTokenExpiryTime(
        LocalDateTime.now().plusMinutes(RESET_TOKEN_VALID_DURATION_MINUTES));
    account.setOtp(null);
  ...
  public Map<String, Object> getAccountStatistics() {
    ...
    List<Account> currentMonthAccounts =
        accountRepository.findByRoleAndCreatedAtBetween(
            Role.MEMBER, currentMonthStart, currentMonthEnd);
    List<Account> previousMonthAccounts =
        accountRepository.findByRoleAndCreatedAtBetween(
            Role.MEMBER, previousMonthStart, previousMonthEnd);
    ...
    double percentageChange =
        previousMonthAccounts.size() == 0
            ? 100.0
            : (dif / (double) previousMonthAccounts.size()) * 100.0;
  ...
  public GlobalResponse<String> deleteAccountById(int accountId) {
    ...
    if (Role.MEMBER.equals(account.getRole())) {
      if (appointmentRepository.existsByMemberIdAndAppointmentStatusOrAppointmentStatus(
          account.getMember().getId(), AppointmentStatus.PENDING, AppointmentStatus.IN_PROGRESS)) {
        throw new RuntimeException(
            "Cannot delete this member account with pending or in progress appointments.");
      }
      ...
      if (appointmentRepository.existsByCoachIdAndAppointmentStatusOrAppointmentStatus(
          account.getCoach().getId(), AppointmentStatus.PENDING, AppointmentStatus.IN_PROGRESS)) {
        throw new RuntimeException(
            "Cannot delete this coach account with pending or in progress appointments.");
      }
      account.setBanned(true);
      account.setActive(false);
      accountRepository.save(account);
