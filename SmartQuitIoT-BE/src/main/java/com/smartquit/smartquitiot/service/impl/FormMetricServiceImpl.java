...
public class FormMetricServiceImpl implements FormMetricService {
  ...
  private static final Set<String> ALLOWED_TRIGGERS =
      Set.of(
          "Morning",
          "After Meal",
          "Gaming",
  ...
  public GetFormMetricResponse getMyFormMetric() {
    ...
    return formMetricMapper.toDTOByGetFormMetric(
        currentQuitPlan.getFormMetric(), currentQuitPlan.getFtndScore());
  }
  ...
  public UpdateFormMetricResponse updateMyFormMetric(UpdateFormMetricRequest req) {
    ...
    if (plan == null) {
      throw new IllegalStateException(
          "Active Quit Plan not found for member " + account.getMember().getId());
    }
    ...
    // Validate input
    Set<String> newInterestNames =
        new HashSet<>(Optional.ofNullable(req.getInterests()).orElse(List.of()));
    if (!newInterestNames.isEmpty()) {
      List<InterestCategory> categories = interestCategoryRepository.findByNameIn(newInterestNames);
      Set<String> found =
          categories.stream().map(InterestCategory::getName).collect(Collectors.toSet());
    ...
    Set<String> newTriggers =
        new HashSet<>(Optional.ofNullable(req.getTriggered()).orElse(List.of()));
    if (!ALLOWED_TRIGGERS.containsAll(newTriggers)) {
      ...
      throw new IllegalArgumentException(
          "Invalid triggers: " + invalid + ". Allowed: " + ALLOWED_TRIGGERS);
    }
    ...
    boolean changedFtndInputs =
        oldSmokeAvgPerDay != req.getSmokeAvgPerDay()
            ...
            || oldNumberOfYearsOfSmoking != req.getNumberOfYearsOfSmoking()
            || // cai nay ko anh huong FTND nhung anh huong den thoi gian cua phase khi tao quitplan
            // nen la dua vao luon
  ...
  private int calculateFTNDScore(
