...
public class QuitPlanServiceImpl implements QuitPlanService {
  ...
  @Transactional
  @Override
  public PhaseBatchMissionsResponse createQuitPlanInFirstLogin(
      CreateQuitPlanInFirstLoginRequest req) {
    ...
    if (account.isFirstLogin()) {
      ...
      PhaseResponse phaseResponse =
          phaseService.generatePhasesInFirstLogin(req, ftndScore, account);
      ...
      formMetric.setEstimatedMoneySavedOnPlan(
          calculateMoneySavedOnPlan(
              req.getStartDate(),
              phaseResponse.getEndDateOfQuitPlan(),
              req.getCigarettesPerPackage(),
              req.getMoneyPerPackage(),
              req.getSmokeAvgPerDay()));
      formMetric.setAmountOfNicotinePerCigarettes(req.getAmountOfNicotinePerCigarettes());
      formMetric.setEstimatedNicotineIntakePerDay(
          calculateNicotineIntakePerDay(
              req.getAmountOfNicotinePerCigarettes(), req.getSmokeAvgPerDay()));
      ...
      List<PhaseDetail> preparedDetails =
          phaseDetailService.generateInitialPhaseDetails(quitPlan, PHASE_PRE);
  ...
  public QuitPlanResponse keepPhaseOfQuitPlan(KeepPhaseOfQuitPlanRequest request) {
    Phase phase =
        phaseRepository
            .findByIdAndQuitPlan_Id(request.getPhaseId(), request.getQuitPlanId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Phase not found for planId="
                            + request.getQuitPlanId()
                            + ", phaseId="
                            + request.getPhaseId()));
    ...
    if (phase.getStatus() == PhaseStatus.FAILED) {
      ...
      throw new RuntimeException(
          "Phase is not in Failed status so cant not keep phase of quit plan");
    }
  ...
  public PhaseBatchMissionsResponse createNewQuitPlan(CreateNewQuitPlanRequest req) {
    ...
    QuitPlan oldQuitPlan =
        quitPlanRepository.findByMember_IdAndIsActiveTrue(account.getMember().getId());
    oldQuitPlan.setActive(false);
    if (oldQuitPlan.getStatus() == QuitPlanStatus.IN_PROGRESS
        || oldQuitPlan.getStatus() == QuitPlanStatus.CREATED) {
    ...
    // get response from ai
    PhaseResponse phaseResponse =
        phaseService.generatePhases(
            newFormMetric.getSmokeAvgPerDay(),
            newFormMetric.getNumberOfYearsOfSmoking(),
            req.getStartDate(),
            oldQuitPlan.getFtndScore(),
            account);
    ...
    // tao phase detail
    List<PhaseDetail> preparedDetails =
        phaseDetailService.generateInitialPhaseDetails(newQuitPlan, "Preparation");

    PhaseBatchMissionsResponse phaseBatchMissionsResponse =
        phaseDetailMissionService.generatePhaseDetailMissionsForPhase(
  ...
  public List<QuitPlanResponse> getHistory() {
    ...
    List<QuitPlan> quitPlans =
        quitPlanRepository.findAllByMember_IdOrderByCreatedAtDesc(account.getMember().getId());
  ...
  public QuitPlanResponse getSpecific(int id) {
    ...
    QuitPlan plan =
        quitPlanRepository
            .findByMember_IdAndId(account.getMember().getId(), id)
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "QuitPlan not found or not belong to this member!"));
    return quitPlanMapper.toResponse(plan);
  }

  private BigDecimal calculateNicotineIntakePerDay(
      BigDecimal amountOfNicotinePerCigarettes, int smokeAvgPerDay) {
    return amountOfNicotinePerCigarettes.multiply(BigDecimal.valueOf(smokeAvgPerDay));
  }

  private BigDecimal calculateMoneySavedOnPlan(
      LocalDate startDate,
      LocalDate endDate,
    ...
    BigDecimal pricePerCigarette =
        moneyPerPackage.divide(BigDecimal.valueOf(cigarettesPerPackage), 2, RoundingMode.HALF_UP);
    // total
  ...
  public AiPredictionResponse getPredictionForCurrentPlan() {
    ...
    Optional<DiaryRecord> latestDiaryOpt =
        diaryRecordRepository.findFirstByMember_IdOrderByDateDesc(member.getId());
    float anxiety = 0f;
    float craving = 0f;
    float mood = 0f;
    float heartRate = 0f;
    ...
    float genderCode =
        (member.getGender() != null && member.getGender().name().equalsIgnoreCase("MALE"))
            ? 1.0f
            : 0.0f;
    float progress = 0;
