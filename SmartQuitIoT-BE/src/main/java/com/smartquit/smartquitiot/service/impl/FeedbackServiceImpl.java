...
public class FeedbackServiceImpl implements FeedbackService {
  ...
  public void createFeedback(int appointmentId, int memberAccountId, FeedbackRequest request) {
    ...
    if (appt.getAppointmentStatus() == null
        || !appt.getAppointmentStatus().name().equalsIgnoreCase("COMPLETED")) {
    ...
    Coach coach =
        coachRepository
            .findByIdForUpdate(appt.getCoach().getId())
  ...
  public Page<FeedbackResponse> getFeedbacksForCoachAccount(int accountId, Pageable pageable) {
    Coach coach =
        coachRepository
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("Coach not found for accountId: " + accountId));
    return getFeedbacksByCoachId(coach.getId(), pageable);
  }

  @Override
  @Transactional(readOnly = true)
  public FeedbackResponse getFeedbackByAppointmentIdForMember(
      int appointmentId, int memberAccountId) {
    ...
    Feedback feedback =
        feedbackRepository
            .findByAppointmentIdAndMemberAccountId(appointmentId, memberAccountId)
            .orElseThrow(
                () -> new IllegalArgumentException("Feedback not found for this appointment"));
