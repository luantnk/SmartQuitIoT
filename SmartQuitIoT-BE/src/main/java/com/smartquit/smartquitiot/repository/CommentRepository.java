...
public interface CommentRepository extends JpaRepository<Comment, Integer> {
  ...
  //            "replies.commentMedia"
  //    })
  //    @Query("SELECT DISTINCT c FROM Comment c WHERE c.parent.id = :parentId ORDER BY c.createdAt
  // ASC")
  ...
  @Query(
      "SELECT c FROM Comment c WHERE c.post.id = :postId AND c.parent IS NULL ORDER BY c.createdAt"
          + " DESC")
  List<Comment> findRootCommentsByPostId(@Param("postId") Integer postId);
