...
public interface FeedbackRepository extends JpaRepository<Feedback, Integer> {
  ...
  @Query(
      "SELECT f.appointment.id FROM Feedback f "
  ...
  @Query("SELECT f FROM Feedback f " + "WHERE f.appointment.id IN :appointmentIds")
  List<Feedback> findByAppointmentIds(@Param("appointmentIds") List<Integer> appointmentIds);

  /**
   * Tìm feedback theo appointmentId và memberAccountId để đảm bảo security. Chỉ member owner của
   * appointment mới có thể xem feedback của mình.
   */
  @EntityGraph(
      attributePaths = {
        "appointment",
  ...
  java.util.Optional<Feedback> findByAppointmentIdAndMemberAccountId(
      @Param("appointmentId") int appointmentId, @Param("memberAccountId") int memberAccountId);
}
