...
public interface ConversationRepository extends JpaRepository<Conversation, Integer> {
  ...
  Optional<Conversation> findDirectConversationBetween(
      @Param("a") int accountA, @Param("b") int accountB);

  @Query(
      "SELECT c FROM Conversation c JOIN c.participants p WHERE p.account.id = :accountId ORDER BY"
          + " c.lastUpdatedAt DESC")
  List<Conversation> findAllByParticipantAccountId(
      @Param("accountId") int accountId, Pageable pageable);
}
