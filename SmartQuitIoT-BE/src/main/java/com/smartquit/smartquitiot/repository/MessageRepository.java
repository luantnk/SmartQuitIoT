...
public interface MessageRepository extends JpaRepository<Message, Integer> {
  ...
  @Query(
      "SELECT m FROM Message m WHERE m.conversation.id = :convId "
          + "AND (:beforeId IS NULL OR m.id < :beforeId) "
          + "ORDER BY m.id DESC")
  List<Message> findMessagesByConversationBeforeId(
      @Param("convId") int convId, @Param("beforeId") Integer beforeId, Pageable pageable);
