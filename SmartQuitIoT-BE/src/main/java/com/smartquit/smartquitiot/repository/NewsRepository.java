...
public interface NewsRepository extends JpaRepository<News, Integer> {
  ...
  @Query(
      "SELECT n FROM News n WHERE LOWER(n.title) LIKE %:query% OR LOWER(n.content) LIKE %:query%"
  ...
  @Query(
      "SELECT n FROM News n WHERE "
          + "(:status IS NULL OR n.status = :status) AND "
          + "(:title IS NULL OR LOWER(n.title) LIKE LOWER(CONCAT('%', :title, '%'))) "
          + "ORDER BY n.createdAt DESC")
  Page<News> findAllWithFilters(
      @Param("status") NewsStatus status, @Param("title") String title, Pageable pageable);
}
