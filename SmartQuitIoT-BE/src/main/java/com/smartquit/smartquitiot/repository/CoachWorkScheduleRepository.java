...
public interface CoachWorkScheduleRepository extends JpaRepository<CoachWorkSchedule, Integer> {
  ...
  @Query(
      "SELECT DISTINCT cws FROM CoachWorkSchedule cws "
  ...
  @Query("SELECT DISTINCT cws.coach.id FROM CoachWorkSchedule cws " + "WHERE cws.date = :date")
  List<Integer> findAllCoachIdsByDate(@Param("date") LocalDate date);

  @Query(
      "SELECT cws FROM CoachWorkSchedule cws "
          + "WHERE cws.coach.id = :coachId AND cws.date = :date")
  List<CoachWorkSchedule> findAllByCoachAndDate(
      @Param("coachId") int coachId, @Param("date") LocalDate date);
  ...
  Optional<CoachWorkSchedule> findByCoachIdAndDateAndSlotIdForUpdate(
      @Param("coachId") int coachId, @Param("date") LocalDate date, @Param("slotId") int slotId);
  ...
  List<CoachWorkSchedule> findAllByDateAndStatusIn(
      @Param("date") LocalDate date, @Param("statuses") List<CoachWorkScheduleStatus> statuses);

  @Query(
      "SELECT DISTINCT cws.date FROM CoachWorkSchedule cws "
          + "WHERE cws.coach.id = :coachId AND cws.date BETWEEN :start AND :end")
  List<LocalDate> findDistinctDatesByCoachIdAndDateBetween(
      @Param("coachId") int coachId, @Param("start") LocalDate start, @Param("end") LocalDate end);

  @Query(
      "select cws from CoachWorkSchedule cws join fetch cws.coach ch where cws.date = :date and"
          + " cws.slot.id = :slotId")
  List<CoachWorkSchedule> findAllByDateAndSlotIdWithCoach(
      @Param("date") LocalDate date, @Param("slotId") int slotId);
}
